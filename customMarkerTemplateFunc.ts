...
/**
 * このサンプルコードにおける、Blazor 側で DataChart のシリーズ (系列) にバインドする型の、JavaScript 側表現です。
 * このサンプルコードにおける Blazor 側のコードでいうと、App.razor 内で定義している、SampleDataType レコード型を表現しています。
 * SampleDataType レコード型のすべてのメンバーを網羅する必要はないため、このサンプルにおけるマーカーの描画のために必要最低限の、
 * Volume プロパティと Color プロパティがあることを示しています。
 * */
type VolumeAndColor = {
    /** App.razor 内で定義している、SampleDataType レコード型の、Volume プロパティに該当します。 */
    Volume: number,
    /** App.razor 内で定義している、SampleDataType レコード型の、Color プロパティに該当します。(C# の System.Drawing.Color 型は、JavaScript 側では、A,R,G,B の各数値として表現されます) */
...
igRegisterScript("customMarkerTemplateFunc", () => {
    return {
         ...
         */
        measure: (measureInfo: IgbMesureInfo<VolumeAndColor>) => {
         ...
         */
        render: (renderInfo: IgbRenderInfo<VolumeAndColor>) => {
